...
const toolCards: DesktopToolCard[] = [
  {
    id: 'video',
    title: '视频播放器',
    description: '添加本地视频目录，按树结构浏览视频，记住上次播放的视频和位置。',
    status: '可用',
    icon: 'video',
  },
  {
  ...
  <section class="desktop-tool-hub">
    <header class="desktop-tool-hub__header">
      ...
      <p class="desktop-tool-hub__summary">
        工具模块独立于阅读模块。这里只放本机工具入口，不展示阅读、最近、收藏或文档目录。
      </p>
    </header>
    ...
    <div class="desktop-tool-hub__grid">
      ...
      >
        ...
        <span
          v-if="!card.disabled"
          class="desktop-tool-hub__action"
        >
          打开工具
...
.desktop-tool-hub {
  ...
  padding: 2rem 2.2rem;
  background:
    radial-gradient(circle at top left, rgba(var(--desktop-accent-rgb), 0.12), transparent 25%),
    linear-gradient(180deg, rgba(var(--desktop-accent-rgb), 0.035), transparent 20%),
    var(--desktop-bg);
}
...
.desktop-tool-hub__card {
  ...
  min-height: 13rem;
  padding: 1.2rem;
  border: 1px solid color-mix(in srgb, var(--desktop-line-strong) 72%, var(--desktop-line));
  border-radius: 22px;
  background:
    linear-gradient(135deg, rgba(var(--desktop-accent-rgb), 0.08), transparent 42%),
...
.desktop-tool-hub__card:hover:not(:disabled) {
  ...
  background:
    linear-gradient(135deg, rgba(var(--desktop-accent-rgb), 0.12), transparent 46%),
    var(--desktop-surface-strong);
}
