# LocalKit Agent Guide

## Product Context

- `LocalKit` 是本地资源工具集，桌面端通过工具中心组织文档阅读、视频播放、Agent 会话清理等工具。
- 文档阅读是工具中心里的核心工具之一，不是唯一主界面；Web 端是文档阅读的静态预览、分享和 GitHub Pages 部署延伸。
...
- 桌面端是主产品形态，采用「工具中心 + 独立工具」结构；当前工具包括文档阅读、视频播放器、Agent 会话清理。
- 文档阅读工具负责文档仓库、文档源、阅读、搜索、编辑；不依赖 `config.yaml`，由本地 SQLite 数据维护。
- 首次进入文档阅读应有默认文档仓库，并指向打包资源中的示例 `docs/`。
- 文档内容支持直接编辑并保存回本地 Markdown 文件。
- 主题支持浅色、暗色、跟随系统、多预设主题色和自定义主题色，各工具共享同一套主题系统。
...
- 页面和工具的大模块布局不要使用卡片化堆叠；优先使用完整页面区块、分栏、面板或工作区布局。卡片只用于小型子组件、重复列表项、详情项、弹窗等局部内容，避免大卡片套小卡片造成审美疲劳。
